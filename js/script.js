...
function addClientLink(clientData) {
  ...
  let isTextChanging = false; // Variable para evitar múltiples clics

  // Función que cambia el texto temporalmente
  function changeTextTemporarily(element, newText, duration) {
    if (isTextChanging) return; // Evita que se ejecute si ya está cambiando el texto
    ...
    isTextChanging = true; // Marcamos que el texto está cambiando

    // Iniciamos un temporizador para restaurar el texto original
    setTimeout(() => {
      element.textContent = originalText; // Restaurar el texto original
      isTextChanging = false; // Permitir de nuevo cambios de texto
    }, duration);
  }
  ...
  ipAddressContainer.addEventListener("click", () => {
    if (!isTextChanging) {
      // Solo permite cambiar el texto si no está en proceso
